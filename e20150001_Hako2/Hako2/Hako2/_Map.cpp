...
static double IIF_ReportRate;

static void IIF_ReportReset(void)
{
	IIF_ReportRate = 0.0;
}
static void IIF_Report(int param1, int param2, int param3)
{
	if(FrameStartTime + 10 < GetCurrTime())
	{
		m_approach(IIF_ReportRate, 1.0, 0.9);

		DrawCurtain();
		SetPrint();
		PE_Border(GetColor(30, 60, 90));
		Print_x(xcout("Loading..."));
//		Print_x(xcout("Initializing... %.3f %% OK", IIF_ReportRate * 100.0)); // old
//		Print_x(xcout("Initializing... %d, %d, %d", param1, param2, param3)); // old
		PE_Reset();
		EachFrame();
	}
}
static void InitItaFlag(void)
{
	...
	for(int loopcnt = 1; ; loopcnt++)
	{
		...
		IIF_Report(loopcnt, vcrlc, hcrlc);
	}
